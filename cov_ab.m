...
%band1 abd band2 
  i=1;
  for ii=1:size(first_band,1) % Number of Rows
    for jj=1:size(first_band,2) % Number of Columns

      data1= first_band(ii,jj);% Extract element from iith row and jjth column for array1
      data2= second_band(ii,jj);% Extract element from iith row and jjth column for array2
